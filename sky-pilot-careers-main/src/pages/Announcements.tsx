    ...
    <div className="min-h-screen">
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {announcements.map((announcement) => (
              ...
              >
                <CardContent className="p-4 border-[1.5px] border-blue-900 rounded-lg">
                  <img
                    src={announcement.poster}
                    alt={announcement.title}
                    className="w-full max-h-96 rounded-lg mb-4 cursor-pointer"
                    onClick={() => setSelectedPoster(announcement.poster)}
                  />
                  <h3 className="text-xl font-semibold mb-2 text-center">{announcement.title}</h3>
                  <p className="text-muted-foreground mb-4 text-center">{announcement.date}</p>
                  <Button className="w-full mb-4" >
                    <Link to="/upload-resume">Apply Now</Link>
                  </Button>
                  <Button
                    onClick={() => setSelectedPoster(announcement.poster)}
                    className="w-full"
      ...
      <Dialog open={!!selectedPoster} onOpenChange={() => setSelectedPoster(null)}>
        <DialogContent className="max-w-[90vw] max-h-[90vh] p-4 bg-background/95 backdrop-blur-sm border shadow-2xl">
          <DialogTitle className="sr-only">Announcement Details</DialogTitle> {/* visually hidden */}
          <DialogDescription className="sr-only">View the announcement poster in full size</DialogDescription>
